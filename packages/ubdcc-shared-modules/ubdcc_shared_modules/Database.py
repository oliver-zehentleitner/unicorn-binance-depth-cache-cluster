...
class Database:
    ...
    def is_empty(self) -> bool:
        if len(self.data["pods"]) == 0 and len(self.data["depthcaches"]) == 0:
            return True
        return False
    ...
    def add_depthcache(
        ...
        depthcache = {
            "CREATED_TIME": self.app.get_unix_timestamp(),
    ...
    def add_depthcache_distribution(
        self,
        exchange: str = None,
        market: str = None,
        pod_uid: str = None,
        scheduled_start_time: float = None,
    ) -> bool:
        if (
            exchange is None
            or market is None
            or pod_uid is None
            or scheduled_start_time is None
        ):
            raise ValueError(
                "Missing mandatory parameter: exchange, pod_uid, market, scheduled_start_time"
            )
        distribution = {
            "CREATED_TIME": self.app.get_unix_timestamp(),
            "LAST_RESTART_TIME": 0,
        ...
        with self.data_lock:
            self.data["depthcaches"][exchange][market]["DISTRIBUTION"][
                pod_uid
            ] = distribution
            self.data["depthcaches"][exchange][market]["DISTRIBUTION_CHANGES"] = (
                self.data["depthcaches"][exchange][market].get(
                    "DISTRIBUTION_CHANGES", 0
                )
                + 1
            )
            self.data["depthcaches"][exchange][market][
                "LAST_DISTRIBUTION_CHANGE"
    ...
    def add_pod(
        ...
        role: str = None,
        ip: str = None,
        api_port_rest: int = None,
        status: str = None,
        ubldc_version: str = None,
        version: str = None,
    ) -> bool:
        if uid is None:
    ...
    def add_credentials(
        self, account_group: str = None, api_key: str = None, api_secret: str = None
    ) -> str:
    ...
    def rebalance_account_group(self, account_group: str = None) -> bool:
        ...
        with self.data_lock:
            ...
            candidates = [
                c for c in credentials.values() if c["ACCOUNT_GROUP"] == account_group
            ]
            dcn_uids = sorted(
                uid
                for uid, pod in self.data.get("pods", {}).items()
            ...
            if before != after:
                self._set_update_timestamp()
                self.app.stdout_msg(
                    f"Rebalanced credentials for '{account_group}': "
    ...
    def assign_credentials(
        self, uid: str = None, account_group: str = None
    ) -> dict | None:
    ...
    def delete(self, key: str = None) -> bool:
        with self.data_lock:
            if key in self.data:
                ...
                self._set_update_timestamp()
                self.app.stdout_msg(
                    f"DB entry deleted: {key}", log="debug", stdout=False
                )
                return True
    ...
    def delete_depthcache(self, exchange: str = None, market: str = None) -> bool:
        ...
        with self.data_lock:
            ...
            self._set_update_timestamp()
        self.app.stdout_msg(
            f"DB depthcaches deleted: {exchange}, {market}", log="debug"
        )
        return True

    def delete_depthcache_distribution(
        self, exchange: str = None, market: str = None, pod_uid: str = None
    ) -> bool:
        ...
        with self.data_lock:
            ...
            self._set_update_timestamp()
        self.app.stdout_msg(
            f"DB depthcache distribution deleted: {exchange}, {market}, {pod_uid}",
            log="debug",
        )
        return True
    ...
    def get_available_dcn_pods(self) -> dict:
        ...
        for uid in self.data["pods"]:
            if self.data["pods"][uid]["ROLE"] == "ubdcc-dcn":
                try:
                    available_dcn_pods[uid] = self.data["nodes"][
                        self.data["pods"][uid]["NODE"]
                    ]["USAGE_CPU_PERCENT"]
                except KeyError:
    ...
    def get_best_dcn(
        self, available_pods: dict = None, excluded_pods: list = None
    ) -> str | None:
        ...
        with self.data_lock:
            ...
            for exchange in self.data["depthcaches"]:
                for market in self.data["depthcaches"][exchange]:
                    for pod_uid in self.data["depthcaches"][exchange][market][
                        "DISTRIBUTION"
                    ]:
                        if (
                            pod_uid == self.app.id["uid"]
                            and self.data["depthcaches"][exchange][market][
                                "DISTRIBUTION"
                            ][pod_uid]["SCHEDULED_START_TIME"]
                            < self.app.get_unix_timestamp()
                        ):
                            responsibilities.append(
                                {
                                    "exchange": exchange,
                                    "market": market,
                                    "refresh_interval": self.data["depthcaches"][
                                        exchange
                                    ][market]["REFRESH_INTERVAL"],
                                    "update_interval": self.data["depthcaches"][
                                        exchange
                                    ][market]["UPDATE_INTERVAL"],
                                }
                            )
    ...
    def get_responsible_dcn_addresses(
        self, exchange: str = None, market: str = None
    ) -> list:
        with self.data_lock:
            responsible_dcn = []
            try:
                for pod_uid in self.data["depthcaches"][exchange][market][
                    ...
                    if (
                        ...
                        responsible_dcn.append(
                            [
                                self.data["pods"][pod_uid]["IP"],
    ...
    def get_worst_dcn(
        self, available_pods: dict | None = None, excluded_pods: list = None
    ) -> str | None:
        if available_pods is None:
            available_pods = self.get_available_dcn_pods()
    ...
    def remove_orphaned_distribution_entries(self) -> bool:
        ...
        with self.data_lock:
            for item in remove_distributions:
                del self.data["depthcaches"][item["exchange"]][item["market"]][
                    "DISTRIBUTION"
                ][item["pod_uid"]]
                self._set_update_timestamp()
        return True
    ...
    def rebalance_credential_assignments_if_needed(self) -> bool:
        ...
        with self.data_lock:
            ...
            assigned_by_group: dict[str, set[str]] = {
                g: set() for g in groups_with_creds
            }
            for c in credentials.values():
    ...
    def manage_distribution(self) -> bool:
        ...
        with self.data_lock:
            for exchange in self.data["depthcaches"]:
                ...
                for market in self.data["depthcaches"][exchange]:
                    ...
                    existing_quantity = len(
                        self.data["depthcaches"][exchange][market]["DISTRIBUTION"]
                    )
                    desired_quantity = self.data["depthcaches"][exchange][market][
                        "DESIRED_QUANTITY"
                    ]
                    if existing_quantity < desired_quantity:
                        ...
                        for i in range(0, add_quantity):
                            best_dcn = self.get_best_dcn(
                                excluded_pods=existing_distribution
                            )
                            ...
                            if best_dcn is not None:
                                ...
                                scheduled_start_time = (
                                    self.app.get_unix_timestamp()
                                    + float(multiplikator) * delayed_start_time
                                )
                                add_distributions.append(
                                    {
                                        "exchange": exchange,
                                        "market": market,
                                        "pod_uid": best_dcn,
                        ...
                        for _ in range(0, remove_quantity):
                            worst_dcn = self.get_worst_dcn(
                                available_pods=available_pods, excluded_pods=exclude_dcn
                            )
        ...
        for item in add_distributions:
            self.add_depthcache_distribution(
        ...
        for item in remove_distributions:
            self.delete_depthcache_distribution(
                exchange=item["exchange"],
                market=item["market"],
                pod_uid=item["pod_uid"],
            )
    ...
    def set(
        self, key: str = None, value: dict | str | float | list | set | tuple = None
    ) -> bool:
        with self.data_lock:
            self.data[key] = value
            self._set_update_timestamp()
        self.app.stdout_msg(
            f"DB entry added/updated: {key} = {value}", log="debug", stdout=False
        )
        return True
    ...
    def update_nodes(self) -> bool:
        ...
        if nodes:
            ...
            self.app.stdout_msg(
                f"Timed update of the DB key 'nodes': Query of the k8s nodes was empty, no "
                f"update is performed!",
                log="error",
                stdout=True,
            )
            return False

    def update_depthcache(
        self,
        desired_quantity: int = None,
        exchange: str = None,
        refresh_interval: int = None,
        ...
        with self.data_lock:
            ...
            if refresh_interval is not None:
                ...
                self._set_update_timestamp()
        self.app.stdout_msg(
            f"DB depthcaches updated: {exchange}, {market}, {desired_quantity}, {update_interval}",
            log="debug",
        )
        return True

    def update_depthcache_distribution(
        self,
        exchange: str = None,
        market: str = None,
        pod_uid: str = None,
        ...
        with self.data_lock:
            if last_restart_time is not None:
                try:
                    dist = self.data["depthcaches"][exchange][market]["DISTRIBUTION"][
                        pod_uid
                    ]
                except KeyError:
                    return False
            ...
            if status is not None:
                try:
                    self.data["depthcaches"][exchange][market]["DISTRIBUTION"][pod_uid][
                        "STATUS"
                    ] = status
                except KeyError:
                    return False
                self._set_update_timestamp()
        self.app.stdout_msg(
            f"DB depthcache distribution updated: {exchange}, {market}, {pod_uid}, {last_restart_time},"
            f" {status}",
            log="debug",
        )
        return True
