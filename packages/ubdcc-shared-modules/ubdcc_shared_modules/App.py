...
class App:
    def __init__(
        ...
        self.rest_server_port_dev_mgmt = (
            mgmt_port if mgmt_port is not None else REST_SERVER_PORT_DEV_MGMT
        )
    ...
    async def get_backup_from_node(self, host, port) -> dict | None:
        data = await self.request(
            f"http://{host}:{port}/ubdcc_mgmt_backup", method="get"
        )
        try:
            data = data["db"]
        except KeyError:
    ...
    async def get_backup_timestamp_from_node(self, host, port) -> float | None:
        data = await self.request(
            f"http://{host}:{port}/ubdcc_mgmt_backup?get_backup_timestamp", method="get"
        )
        try:
            ...
            for selection_uid in selection:
                if selection_uid == uid:
                    if dcn_unused_longest_time is None:
                        ...
                        if (
                            self.dcn_usage[dcn_unused_longest_time]
                            > self.dcn_usage[uid]
                        ):
        ...
        if self.k8s_client is not None:
            ...
            for node in k8s_nodes.items:
                ...
                try:
                    ...
                    self.stdout_msg(
                        f"Error when querying the K8s nodes: {error_msg}", log="error"
                    )
                    return {}
                ...
                if cpu_usage.endswith("m"):
                    ...
                    cpu_usage_milli = (
                        int(cpu_usage) * 1000
                    )  # Assume no unit means cores, convert to milli
                ...
                memory_usage_bytes = (
                    int(memory_usage[:-2]) * 1024
                )  # Assuming 'Ki' suffix
                memory_capacity_bytes = (
                    int(memory_capacity[:-2]) * 1024
                )  # Assuming 'Ki' suffix
    ...
    def get_k8s_runtime_information(self):
        try:
            ...
            self.pod_info = self.k8s_client.read_namespaced_pod(
            ...
            self.dev_mode = True
        self.id["name"] = (
            self.generate_string(random.randint(10, 15))
            if self.dev_mode
            else self.pod_info.metadata.name
        )
        self.id["uid"] = (
            self.generate_string(random.randint(20, 20))
            if self.dev_mode
            else self.pod_info.metadata.uid
        )
        self.id["namespace"] = (
            self.generate_string(random.randint(10, 15))
            if self.dev_mode
            else self.pod_info.metadata.namespace
        )
        self.id["node"] = (
            self.generate_string(random.randint(15, 20))
            if self.dev_mode
            else self.node_info.metadata.uid
        )
        self.id["labels"] = (
            self.generate_string(random.randint(10, 15))
            if self.dev_mode
            else self.pod_info.metadata.labels
        )
    ...
    async def send_backup_to_node(self, host, port) -> dict:
        return await self.request(
            f"http://{host}:{port}/ubdcc_mgmt_backup",
            method="post",
    ...
    def set_api_rest_port(self):
        if self.dev_mode:
            ...
            if self.info["name"] == "ubdcc-dcn":
                ...
                raise ValueError(
                    f"Not able to choose the right rest server port for app '{self.info['name']}'"
                )
        else:
            # PRODUCTIVE MODE!!!
    ...
    def sigterm_handler(self, signal, frame) -> None:
        self.sigterm = True
        self.stdout_msg(
            f"Processing SIGTERM - signal: {signal} - frame: {frame}",
            log="debug",
            stdout=False,
        )
        self.stdout_msg(
            f"Received SIGTERM, performing graceful shutdown ...", log="warn"
        )
    ...
    def sort_dict(input_dict: dict, reverse: bool = False) -> dict:
        sorted_items = sorted(
            input_dict.items(), key=lambda item: item[0], reverse=reverse
        )
    ...
    def start(self) -> None:
        ...
        info = (
            f"Starting {self.info['name']}_{self.info['version']}_{self.info['build_type']} by "
            f"{self.info['author']} ..."
        )
        ...
        try:
            ...
            if exception_shutdown is True:
                if exception_shutdown_error:
                    self.stdout_msg(
                        f"ERROR: {exception_shutdown_error}", log="critical"
                    )
                self.stop_call()
                self.stdout_msg(
                    f"The system was gracefully shut down after a critical error was encountered.",
                    log="info",
                )
                sys.exit(1)
            else:
                ...
                self.stop_call()
                self.stdout_msg(
                    f"Gracefully shutdown finished! Thank you and good bye ...",
                    log="info",
                )
                sys.exit(0)
    ...
    async def ubdcc_get_responsible_dcn_addresses(
        self, exchange: str = None, market: str = None
    ):
        self.stdout_msg(
            f"Get responsible DCN addresses for {market} on {exchange} ...", log="info"
        )
        endpoint = "/ubdcc_get_responsible_dcn_addresses"
        host = self.get_cluster_mgmt_address()
        query = f"?exchange={exchange}&" f"market={market}"
        url = host + endpoint + query
        result = await self.request(url=url, method="get")
        if result.get("error_id") is None and result.get("error") is None:
            self.stdout_msg(
                f"Successfully caught responsible DCN addresses for {market} on {exchange}!",
                log="info",
            )
            return result
        else:
            self.stdout_msg(
                f"Can not catch responsible DCN addresses: {result}", log="error"
            )
            return None
    ...
    async def ubdcc_node_cancellation(self):
        ...
        if result.get("error_id") is None and result.get("error") is None:
            ...
            self.stdout_msg(
                f"Error during node cancellation: {result.get('error')}", log="warn"
            )
            return False
            ...
            self.stdout_msg(
                f"Error during node sync: {result.get('error_id')} - {result.get('message')}",
                log="error",
            )
            return False

    async def ubdcc_node_registration(
        self, ubldc_version: str = None, retries: int = 30
    ) -> bool:
        ...
        while loops < retries and self.is_shutdown() is False:
            ...
            if result.get("error_id") is None and result.get("error") is None:
                ...
                self.stdout_msg(
                    f"Mgmt Service is not ready yet! Waiting a few seconds till retry!",
                    log="warn",
                )
                await asyncio.sleep(self.mgmt_is_ready_time)
            await asyncio.sleep(3)
        self.stdout_msg(
            f"Error during node registration: {result.get('error_id')} - {result.get('error')}",
            log="error",
        )
        return False
    ...
    async def ubdcc_node_sync(self) -> bool:
        ...
        if result.get("error_id") is None and result.get("error") is None:
            ...
            self.stdout_msg(
                f"Error during node sync: {result.get('error')}", log="warn"
            )
            return False
    ...
    async def ubdcc_assign_credentials(self, account_group: str = None) -> dict | None:
        ...
        if result.get("error_id") is not None or result.get("error") is not None:
            self.stdout_msg(
                f"Could not assign credentials for '{account_group}': {result}",
                log="warn",
            )
            return None
        return result.get("credential")

    async def ubdcc_update_depthcache_distribution(
        self,
        exchange: str = None,
        market: str = None,
        ...
        while self.is_shutdown() is False:
            ...
            if result.get("error_id") is None and result.get("error") is None:
                self.stdout_msg(
                    f"DepthCache distribution update succeeded!", log="info"
                )
                return True
            elif result.get("error_id") == "#1023":
                self.stdout_msg(
                    f"Error during DepthCache distribution update: {result}",
                    log="error",
                )
                return False
            else:
