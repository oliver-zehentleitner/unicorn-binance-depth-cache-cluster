...
class RestEndpointsBase:
    ...
    def create_cluster_info_response(self) -> dict:
        if self.app.data.get("db") is None:
            ...
            response = {
                "db": {
                    "credentials": self.app.data["db"].get_credentials_list(
    ...
    def create_debug_response(
        ...
        result = {
            "cluster_execution_time": time.time() - process_start_time,
            "request_time": 0,
            "transmission_time": 0,
            "url": url,
            "used_pods": used_pods,
    ...
    def create_depthcache_info_response(
        ...
        if self.app.data.get("db") is None:
            ...
            response = {
                "depthcache_info": self.app.data["db"].get_depthcache_info(
    ...
    def get_error_response(
        ...
        if process_start_time is not None:
            response["debug"] = self.create_debug_response(
    ...
    def is_ready(self):
        try:
            if self.app.data["is_ready"] is True:
                ...
                if (
                    self.app.data["start_timestamp"] + self.app.mgmt_is_ready_time
    ...
    async def test(self, request: Request):
        ...
        if self.app.pod_info is not None:
            pod = {
                "name": self.app.pod_info.metadata.name,
    ...
    async def shutdown(self, request: Request):
        ...
        return self.get_ok_response(
            event=event,
            params={"message": f"Shutting down pod '{self.app.id['name']}'..."},
        )

    def throw_error_if_mgmt_not_ready(self, request: Request, event: str = None):
        if self.is_ready() is False:
            self.app.stdout_msg(
                f"Mgmt Service is not ready yet! Telling '{request.query_params.get('uid')}' to come "
                f"back later!",
                log="warn",
            )
    ...
    async def ubdcc_mgmt_backup(self, request: Request):
        ...
        if not request_body.decode("utf-8").strip('"'):
            ...
            if backup_timestamp is not None:
                return self.get_ok_response(
                    event=event, params={"timestamp": self.app.get_backup_timestamp()}
                )
            # provide the backup data for restore
            self.app.stdout_msg(f"Provided backup database!", log="info")
            return self.get_ok_response(
                event=event, params={"db": self.app.ubdcc_mgmt_backup}
            )
        else:
            ...
            if self.app.info["name"] != "ubdcc-mgmt":
                ...
                try:
                    ...
                    self.app.stdout_msg(
                        f"Database not available: {error_msg}",
                        log="debug",
                        stdout=False,
                    )
            self.app.ubdcc_mgmt_backup = json.loads(request_body.decode("utf-8"))
            return self.get_ok_response(
                event=event, params={"message": "The backup has been saved!"}
            )
