...
class RestEndpoints(RestEndpointsBase):
    ...
    async def _get_depthcache_data(self, request: Request, event=None, endpoint=None):
        ...
        responsible_dcn = await self.app.ubdcc_get_responsible_dcn_addresses(
        ...
        if responsible_dcn is None:
            ...
            addresses = self.app.data["db"].get_responsible_dcn_addresses(
                exchange=exchange, market=market
            )
        else:
        ...
        result_errors = []
        failover_errors = []
        first_choice_dcn = self.app.get_dcn_uid_unused_longest_time(
            selection=[address[2] for address in addresses]
        )
        ...
        for address, port, uid in addresses:
            self.app.stdout_msg(
                f"Connecting http://{address}:{port}/{endpoint}{query} ..."
            )
            ...
            if result.get("error") is None and result.get("error_id") is None:
                if str(request.query_params.get("debug")).lower() == "true":
                    ...
                    result["debug"] = self.create_debug_response(
                        process_start_time=process_start_time,
                        url=request_url,
                        used_pods=used_pods,
                    )
        ...
        return self.get_error_response(
            event=event,
            error_id="#5000",
            message=f"No DCN has responded to the request!",
    ...
    async def get_asks(self, request: Request):
        event = "GET_ASKS"
        endpoint = "/get_asks"
        return await self._get_depthcache_data(
            request=request, event=event, endpoint=endpoint
        )
    ...
    async def add_credentials(self, request: Request):
        return await self._proxy_to_mgmt(
            request=request,
            event="ADD_CREDENTIALS",
            endpoint="/add_credentials",
            allow_post=True,
        )
