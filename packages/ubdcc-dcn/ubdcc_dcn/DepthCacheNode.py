...
class DepthCacheNode(ServiceBase):
    def __init__(self, cwd=None, mgmt_port=None, log_level=None):
        ...
        super().__init__(
            app_name="ubdcc-dcn", cwd=cwd, mgmt_port=mgmt_port, log_level=log_level
        )
    ...
    def _ldcs_for_account_group(self, account_group: str):
        ...
        for exchange, by_interval in self.app.data.get(
            "depthcache_instances", {}
        ).items():
    ...
    async def _sync_credentials(self) -> None:
        ...
        for account_group in used_account_groups:
            credential = await self.app.ubdcc_assign_credentials(
            ...
            for ldc in self._ldcs_for_account_group(account_group):
                try:
                    ...
                    self.app.stdout_msg(
                        f"set_credentials() failed for '{account_group}': "
                        f"{error_msg}",
                        log="error",
                    )
            ...
            if new_id is not None:
                self.app.stdout_msg(
                    f"Assigned credential '{new_id}' for account_group "
                    f"'{account_group}'.",
                    log="info",
                )
            else:
                self.app.stdout_msg(
                    f"No credential available for account_group "
    ...
    async def main(self):
        ...
        while self.app.is_shutdown() is False:
            ...
            self.app.stdout_msg(
                f"Local DepthCaches: {self.app.data['local_depthcaches']}",
                log="debug",
                stdout=False,
            )
            self.app.stdout_msg(
                f"Responsibilities: {self.app.data['responsibilities']}",
                log="debug",
                stdout=False,
            )
            for dc in self.app.data["responsibilities"]:
                ...
                if dc not in self.app.data["local_depthcaches"]:
                    ...
                    if (
                        self.app.data["depthcache_instances"].get(dc["exchange"])
                        is None
                    ):
                        self.app.data["depthcache_instances"][dc["exchange"]] = {}
                    if (
                        self.app.data["depthcache_instances"][dc["exchange"]].get(
                            dc["update_interval"]
                        )
                        is None
                    ):
                        ...
                        if dc["update_interval"] is not None:
                            kwargs["depth_cache_update_interval"] = dc[
                                "update_interval"
                            ]
            ...
            for dc in self.app.data["local_depthcaches"]:
                ...
                if dc not in self.app.data["responsibilities"]:
                    ...
                    if stop_depthcaches.get(dc["exchange"]) is None:
                        stop_depthcaches[dc["exchange"]] = {
                            dc["update_interval"]: {"markets": [dc["market"]]}
                        }
                    else:
                        if (
                            stop_depthcaches[dc["exchange"]].get(dc["update_interval"])
                            is None
                        ):
                            stop_depthcaches[dc["exchange"]] = {
                                dc["update_interval"]: {"markets": [dc["market"]]}
                            ...
                            stop_depthcaches[dc["exchange"]][dc["update_interval"]][
                                "markets"
                            ].append(dc["market"])
            ...
            for exchange in stop_depthcaches:
                for update_interval in stop_depthcaches[exchange]:
                    try:
                        self.app.data["depthcache_instances"][exchange][
                            update_interval
                        ].stop_depthcache(
                            markets=stop_depthcaches[exchange][update_interval][
                                "markets"
                            ]
                        )
                    except DepthCacheNotFound as error_msg:
                        self.app.stdout_msg(
                            f"DepthCache not found: {error_msg}", log="error"
                        )
        self.app.stdout_msg(f"Stopping all DepthCache instances ...", log="error")
        for dc in self.app.data["local_depthcaches"]:
            for update_interval in self.app.data["depthcache_instances"][
                dc["exchange"]
            ]:
                self.app.data["depthcache_instances"][dc["exchange"]][
                    update_interval
