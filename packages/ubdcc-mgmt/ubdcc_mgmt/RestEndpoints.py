...
class RestEndpoints(RestEndpointsBase):
    ...
    async def create_depthcache(self, request: Request):
        ...
        try:
            ...
            return self.get_error_response(
                event=event, error_id="#1017", message=str(error_msg)
            )
        if result is True:
            # Todo:
            # Add Option to run:
            ...
            return self.get_error_response(
                event=event, error_id="#1018", message="An unknown error has occurred!"
            )
    ...
    async def get_depthcache_info(self, request: Request):
        ...
        if not response["depthcache_info"]:
            return self.get_error_response(
                event=event,
                error_id="#7000",
                message=f"DepthCache '{market}' for " f"'{exchange}' not found!",
            )
        return self.get_ok_response(event=event, params=response)
    ...
    async def stop_depthcache(self, request: Request):
        ...
        try:
            ...
            return self.get_error_response(
                event=event, error_id="#1020", message=str(error_msg)
            )
        if result is True:
            return self.get_ok_response(event=event)
        else:
            return self.get_error_response(
                event=event, error_id="#1021", message="An unknown error has occurred!"
            )
    ...
    async def ubdcc_node_cancellation(self, request: Request):
        ...
        if uid is None:
            return self.get_error_response(
                event=event, error_id="#1004", message="Missing required parameter: uid"
            )
        ...
        if result is True:
            ...
            return self.get_error_response(
                event=event, error_id="#1008", message="An unknown error has occurred!"
            )
    ...
    async def ubdcc_node_registration(self, request: Request):
        ...
        if result is True:
            await self.app.send_backup_to_node(
                host=request.client.host, port=api_port_rest
            )
            return self.get_ok_response(event=event)
        else:
            return self.get_error_response(
                event=event, error_id="#1009", message="An unknown error has occurred!"
            )
    ...
    async def ubdcc_node_sync(self, request: Request):
        ...
        if not self.db.exists_pod(uid=uid) and self.db.is_empty() is True:
            backup = await self.app.get_backup_from_node(
                host=request.client.host, port=api_port_rest
            )
            if backup is not None:
                ...
                for pod in backup["pods"]:
                    ...
                    timestamp = await self.app.get_backup_timestamp_from_node(
                        host=backup["pods"][pod]["IP"],
                ...
                if source_uid != uid:
                    backup = await self.app.get_backup_from_node(
                        host=source_ip, port=source_port
                    )
                if backup is not None:
                    ...
                    self.app.stdout_msg(
                        f"Loaded database from pod '{source_uid}'!", log="info"
                    )
        if not self.db.exists_pod(uid=uid):
            return self.get_error_response(
                event=event,
        ...
        result = self.db.update_pod(
            uid=uid, node=node, ip=request.client.host, status=status
        )
        pod = self.db.get_pod_by_uid(uid=uid)
        if result is True:
            await self.app.send_backup_to_node(
                host=request.client.host, port=pod["API_PORT_REST"]
            )
            return self.get_ok_response(event=event)
        else:
            return self.get_error_response(
                event=event, error_id="#1010", message="An unknown error has occurred!"
            )
    ...
    async def add_credentials(self, request: Request):
        ...
        try:
            credential_id = self.db.add_credentials(
                account_group=account_group, api_key=api_key, api_secret=api_secret
            )
        except ValueError as error_msg:
            return self.get_error_response(
                event=event, error_id="#1032", message=str(error_msg)
            )
    ...
    async def remove_credentials(self, request: Request):
        ...
        if credential_id is None:
            return self.get_error_response(
                event=event, error_id="#1033", message="Missing required parameter: id"
            )
    ...
    async def ubdcc_update_depthcache_distribution(self, request: Request):
        ...
        result = self.db.update_depthcache_distribution(
            exchange=exchange,
            market=market,
            pod_uid=pod_uid,
        ...
        if result is True:
            ...
            return self.get_error_response(
                event=event,
                error_id="#1023",
                message=f"DepthCache '{exchange} {market} " f"{pod_uid}' not found!",
            )
